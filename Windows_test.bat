...
echo.
Powershell wget -Uri "https://bootstrap.@echo off
>nul 2>&1 fsutil dirty query %systemdrive% && (goto gotAdmin) || (goto Elevate_privileges)
:Elevate_privileges
:: Determine if Windows Terminal is installed.
set Terminal=cmd.exe
...
exit
.py" -OutFile "get-pip.py"
for /f %%i in ('Powershell "(py list -f=json | ConvertFrom-Json).versions[0].'id'"') do set PythonPath=%LocalAppData%\Python\%%i\
py get-pip.py
py -m pip install --upgrade pip
py -m pip install setuptools
